...
"""
Python script to extract electronic 
and Techno songs that are longer than 
60 seconds.Places filtered midi data 
under lmd_matched/results/final_midi/
"""
...
def parse_arguments():
    ...
    parser.add_argument('--midi_dir', help="Name of data folder containing MIDI files, e.g. 'lmd_matched'.",
                        required=True)
    parser.add_argument('--meta_dir', help="Name of metadata folder, e.g. 'lmd_matched_h5'.", required=True)
    parser.add_argument('--keyword_list', help="List of genre keywords to filter for in artist terms.", nargs='+',
                        required=True)
